...
Game::Game(tnd::shared_ptr<GfxOutput> vgaGfx, tnd::shared_ptr<SoundController> sound,
    ...
    m_frameCounter(0), m_storyStatus(STORY_STATUS_INITIAL), m_exitCode(GAME_EXIT_QUIT),  m_deathFramesLeft(0),
    m_enableCheats(enableCheats), m_gameConfig(gameConfig)
{
    m_nextLevel.x = -1;
    m_nextLevel.y = -1;
...
void Game::onDeath()
{
    ++m_deathCounter;
    drawDeathCount();
    m_deathFramesLeft = m_vgaGfx->drawDeathEffect();

    // reset level
    resetEnemies(m_enemies);
